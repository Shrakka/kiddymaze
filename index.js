...
let TILE_SIZE;
let mazeContainer;
let maze, path, character;
let instructions, instructionsContainer;
...
function setupMazeScreen() {
	...
	maze = new Maze(MAZES[1]);
	character = 
	TILE_SIZE = Math.min(
		Math.floor(window.innerWidth * 0.6 / maze.getSize().col), 
		Math.floor(window.innerHeight / maze.getSize().lig)
	);
	drawMaze();
	drawCharacter();
	bindKeys();
...
function drawMaze() {
	...
	mazeContainer.y = Math.floor( (window.innerHeight - TILE_SIZE * maze.getSize().lig) / 2 )
...
function setupInstructionScreen() {
	...
	title.width = Math.floor(innerWidth*0.4);
	title.x = Math.floor(innerWidth*0.6);
	app.stage.addChild(title);

	const forwardButton = new Sprite(loader.resources["images/sprites/b_forward.png"].texture) 
	forwardButton.width = Math.floor(innerWidth*0.1);
	forwardButton.anchor.set(0.5);
	forwardButton.x = Math.floor(innerWidth*0.8);
	forwardButton.y = Math.floor(innerHeight*0.7);
	...
	runButton.width = Math.floor(innerWidth*0.1);
	runButton.anchor.set(0.5);
	runButton.x = Math.floor(innerWidth*0.8);
	runButton.y = Math.floor(innerHeight*0.9);
	...
	instructionsContainer.x = Math.floor(innerWidth*0.6);
